...
{
public:
   QormDatabaseException(const QSqlError &error) : runtime_error(error.text().toLocal8Bit().data()) {}
};

class QMETAORM_LIBRARY_API QormConnectToDatabaseException : public QormDatabaseException
{
public:
   QormConnectToDatabaseException(const QSqlError &error) : QormDatabaseException(error)
   {
      qDebug() << error;
   }
};
...
{
public:
   QormTransactionException(const QSqlError &error) : QormDatabaseException(error) {}
};

class QMETAORM_LIBRARY_API QormQueryException : public QormDatabaseException
{
public:
   QormQueryException(const QSqlError &error) : QormDatabaseException(error)
   {
      qDebug() << error;
   }
};
...
{
public:
   QormCouldNotPrepareQueryException(const QSqlError &error) : QormQueryException(error) {}
};

class QMETAORM_LIBRARY_API QormCouldNotExecuteQueryException : public QormQueryException
{
public:
   QormCouldNotExecuteQueryException(const QSqlError &error) : QormQueryException(error) {}
};
...
{
public:
   QormCouldNotQueryNextSequenceValueException(const QSqlError &error) : QormQueryException(error) {}
};
